...
1.  Standard key-value pairs (`block_mapping_pair`) where the key matches any of the dependency triggers (`uses`, `image`, `pipe`, `include`, `ref`, `task`, `template`, `bundle`, `container`).
...
| :--- | :--- | :--- | :--- |
| **GitHub** | `.github/workflows`, `.github/actions`, `action.yml`, `action.yaml` | `uses`, `image`, `container` | GitHub Action or OCI Image |
...
| **Forgejo / Gitea**| `.forgejo/workflows`, `.gitea/workflows` | `uses`, `image`, `container` | Forgejo Action or OCI Image |
